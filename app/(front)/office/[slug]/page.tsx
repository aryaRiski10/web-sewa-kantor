...
import { offices } from "@/app/features/offices/data/offices.mock";
import { notFound } from "next/navigation";
import Image from "next/image";
import OfficeHeader from "@/app/features/offices/components/OfficeHeader";
import OfficeContact from "@/app/features/offices/components/OfficeContact";
import OfficeFeatures from "@/app/features/offices/components/OfficeFeatures";

type Props = {
    params: Promise<{slug: string}>
}
export default async function OfficeSinglePage( { params }: Props) {
              ...
              <div className="flex flex-col gap-[6px]">
                <div className="flex items-center gap-1">
                  {Array.from({length: 5}).map((_,i) => (
                    <Image key={i} src={i < Math.floor(office.rating)
                      ? "/assets/images/icons/Star 1.svg"
                      : "/assets/images/icons/Star 5.svg"
                    }
                    className="w-5 h-5"                    
                    alt="icon"
                    width={20}
          ...
          <div className="w-[392px] max-md:w-full flex flex-col shrink-0 gap-[30px]">
            <div className="flex flex-col rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
              {office.isBooked ? (
                <div>
                  ...
                  <p className="font-semibold mt-1">For {office.duration}</p>
                </div>
              ) : (
                <div>
                  <p className="font-extrabold text-[32px] leading-[48px] text-[#0D903A]">Sold Out</p>
                  <p className="font-semibold mt-1">Please choose another office</p>
                </div>
              )}
              ...
              <div className="flex flex-col gap-[14px]">
                ...
                >
                  ...
                  <p className="font-semibold text-center text-[#FF0000]">This office is not available for booking.</p>
                )}
            ...
            <div className="flex flex-col rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[20px] bg-white">
              ...
              <div className="flex flex-col gap-[30px]">
                ...
                {office.salesContact.map((contact, i) => (
                  <OfficeContact key={i} contact={contact} />
                ))}
